...
//                          print("onchange userFlag:\(userFlag)")
      //                userFlag = authManager.userFlag
        ...
        .onChange(of: showCompletionMessage) { newValue in
            guard newValue, !hasHandledCompletion else { return }
            ...
            if playerHP <= 0 {
                ...
                if let currentUserId = authManager.currentUserId {
                    authManager.subtractRankMatchPoints(for: currentUserId, points: 10, onSuccess: {
                        print("@@@@@@@@@@@@@@@@@@@@@@@2")
                    }, onFailure: { error in
                    })
                }
...
//                }
                ...
                DispatchQueue.global(qos: .background).async {
                    ...
                    if let currentUserId = authManager.currentUserId {
                        authManager.addRankMatchPoints(for: currentUserId, points: 10, onSuccess: {
                            print("@@@@@@@@@@@@@@@@@@@@@@@3")
                        }, onFailure: { error in
                        })
                    }
