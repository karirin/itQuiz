...
struct QuizAppliedIncorrectAnswerListView: View {
    ...
    var body: some View {
        VStack{
            if isDataLoaded {
                ...
                QuizView(quizzes: shuffledQuizList, quizLevel: .incorrectAppliedAnswer, authManager: authManager, audioManager: audioManager, isPresenting: $isPresenting, interstitial: sharedInterstitial)
            } else {
                ActivityIndicator()
            }
        }
        .onAppear {
            ...
            fetchAppliedIncorrectAnswers(userId: authManager.currentUserId!) { quizList in
...
func fetchAppliedIncorrectAnswers(userId: String, completion: @escaping ([QuizQuestion]) -> Void) {
...
struct QuizAppliedIncorrectAnswerListView_Previews: PreviewProvider {
    static var previews: some View {
        QuizAppliedIncorrectAnswerListView(isPresenting: .constant(false))
    }
}
