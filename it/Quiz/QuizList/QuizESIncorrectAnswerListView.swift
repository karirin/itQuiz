...
struct QuizESIncorrectAnswerListView: View {
    ...
    var body: some View {
        VStack{
            if isDataLoaded {
                ...
                QuizView(quizzes: shuffledQuizList, quizLevel: .incorrectESAnswer, authManager: authManager, audioManager: audioManager, isPresenting: $isPresenting, interstitial: sharedInterstitial)
            } else {
                ActivityIndicator()
            }
        }
        .onAppear {
            ...
            fetchESIncorrectAnswers(userId: authManager.currentUserId!) { quizList in
...
func fetchESIncorrectAnswers(userId: String, completion: @escaping ([QuizQuestion]) -> Void) {
...
struct QuizESIncorrectAnswerListView_Previews: PreviewProvider {
    static var previews: some View {
        QuizESIncorrectAnswerListView(isPresenting: .constant(false))
    }
}
