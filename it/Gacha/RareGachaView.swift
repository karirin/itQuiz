...
struct RareGachaView: View {
    ...
    var body: some View {
        ...
        .alert("コインが足りません", isPresented: $showUnCoinModal) {
            if authManager.rareGachaTicketCount > 0 {
                Button("チケットで引く") {
    ...
    private var gachaMainView: some View {
        GachaLobbyScreen(
            gachaTitle: "幸福ガチャ",
            heroImageName: "レアガチャ",
            ticketType: .rareGachaTicket,
            ticketCount: authManager.rareGachaTicketCount,
            userMoney: userMoney,
    ...
    private func startGachaWithTicket() {
        guard authManager.rareGachaTicketCount > 0 else {
            ticketErrorMessage = "幸福ガチャチケットがありません。"
            showTicketErrorAlert = true
            refreshPlayerResources()
            return
        }

        authManager.consumeItem(.rareGachaTicket) { success in
            guard success else {
